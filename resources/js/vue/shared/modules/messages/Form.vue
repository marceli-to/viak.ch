...
<form @submit.prevent="submit" v-if="isFetched">
  <article-text>
    <template #aside>
      <h1 :class="[isExpert ? 'xs:hide' : '', '']">{{ __('Nachricht erstellen') }}</h1>
      <p class="text-small sm:mt-3x">{{ __('Sende eine Nachricht an alle Studenten dieses Kurses.') }}</p>
      <div class="sm:mt-5x md:mt-10x">
        <router-link :to="{ name: 'event-show', params: { uuid: $route.params.uuid } }" class="icon-arrow-right:below" v-if="isAdmin">
          <span>{{ __('Zurück') }}</span>
          <icon-arrow-right :size="'sm'" />
        </router-link>
        <router-link :to="{ name: 'expert-course-event' }" class="icon-arrow-right:below" v-if="isExpert">
          <span>{{ __('Zurück') }}</span>
          <icon-arrow-right :size="'sm'" />
        </router-link>
      </div>
...
export default {
  ...
  mounted() {
    this.isAdmin =  this.$route.name == 'event-message' ? true : false;
    this.isExpert = this.$route.name == 'expert-course-event-message' ? true : false;
  ...
  methods: {
    ...
    submit() {
      ...
      this.axios.post(this.routes.store, this.data).then(response => {
        NProgress.done();
        this.isLoading = true;
        let redirect = this.isAdmin ? 'event-show' : 'expert-course-event';
        this.$router.push({ name: redirect, params: { uuid: this.$route.params.uuid } });
      });
    },
